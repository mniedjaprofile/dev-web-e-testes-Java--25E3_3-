...
public class Exercicio8PutAtualizarEntidade {
    ...
    public static void main(String[] args) {
        ...
        atualizarComPut(url, jsonInput);

        verificarEntidade(url);
    }

    private static void atualizarComPut(String url, String jsonInput) {
        try {
            ...
            System.out.println("Código de Status HTTP (PUT): " + statusCode);
    ...
    private static void verificarEntidade(String url) {
        try {
            ...
            if (response.toString().contains("\"name\":\"atualizado\"")) {
                System.out.println("A entidade foi atualizada com PUT!");
            } else {
                System.out.println("O campo 'name' não corresponde ao esperado.");
            }

        } catch (IOException e) {
            System.err.println("Erro na verificação GET: " + e.getMessage());
        }
    }
}
